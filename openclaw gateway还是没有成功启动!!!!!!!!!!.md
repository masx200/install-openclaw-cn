# OpenClaw Gateway 启动失败原因分析
  ...
  RPC probe: failed - gateway closed (1006 abnormal closure)
  ...
  安全设计考虑：
  1. 防止意外暴露 - 如果用户没有明确指定模式，Gateway 不会启动，避免服务以错误的配置暴露到公网
  ...
  1. 查看原始错误 - openclaw gateway status 显示的是摘要，直接运行命令才能看到完整错误
